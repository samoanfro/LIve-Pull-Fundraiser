...
- [ ] **Business model clarification from the user (2026-09-15): SHi donates the inventory (packs/boxes) to Live Pull, Live Pull sells it, and SHi pays Live Pull a fee for the selling service.** This is the reverse of what the current schema assumes — right now `organizations` represents one tenant that both owns the campaign and receives all proceeds. Worth revisiting whether "Live Pull" and "SHi" are the same `organizations` row, two related rows, or whether a fee/commission needs its own tracked field once this relationship is finalized. Doesn't block current functionality (single-org MVP), but affects future reporting/revenue-split logic and possibly disclosure language on product pages (who's "the nonprofit beneficiary" if Live Pull itself isn't the 501(c)(3)?).
...
- Design system switched from the original invented dark/amber theme to SHi's actual light/navy/teal/orange brand palette. Admin pages only got a light branding touch (logo mark, accent nav color) — the actual admin page bodies (tables, forms) are still the plain default styling. A full admin design pass is not done; let me know if you want that too.
- [ ] **Demo inventory added for click-through testing (2026-09-15):** 20 sealed "2026 Collectible Hobby Pack" units ($29, PACK-000005–000024) and 1 "Rare Chase Box" unit ($99, PACK-000025) under the Fall Fundraiser 2026 campaign. All added directly via SQL against the linked Supabase project (not through the admin UI, since sandbox browser can't complete Supabase magic-link sign-in). Replace with real product names, pricing, and photos once you have physical inventory — these are seed/demo values.
- [ ] **Home page hero illustration (`app/hero-art.tsx`) is a placeholder** — original abstract shapes in brand colors, not real product photography, since Pokemon/One Piece/Magic box art and logos are trademarked and can't be reproduced without a license. Swap this out for real photos once you have physical inventory to photograph (per spec §32's "clear product photography" guidance) — the component is isolated specifically to make that swap easy.
