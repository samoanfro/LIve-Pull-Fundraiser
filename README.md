...
**Status: Phase 9 (Reporting & CSV Export) done.** Admin has a Reports page covering Campaign/Financial (gross sales, orders, packs sold, AOV, refunds, product cost, estimated net funds raised), Operations (opening/shipping/donation-back queue counts, unresolved exceptions), and Inventory (received/available/reserved/sold/opened/damaged/missing/adjusted) — each with CSV export. Phase 8 (Audit History & Exception Handling) is done: every sensitive state transition writes an append-only, before/after audit row that no role can update or delete, and a real pre-existing authorization gap in several Phase 4/6/7 database functions was found and fixed (confirmed via a real unauthenticated REST call). Phase 4 (Stripe Checkout & Inventory Reservations) is built and verified at the database level, but **still not live-tested against a real Stripe account** — `STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET` remain blank in `.env.local`. See `TODO.md` for the full running list of open items, `CLAUDE.md` and `PRODUCT_BUILD_SPEC.md` for the source of truth before any architectural changes.
